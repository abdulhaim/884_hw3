...
parser.add_argument('--env', default='PusherEnv')
...
class PPO:
    ...
    def update_joint(self, memory):
        ...
        for _ in range(self.K_epochs):
            ...
            surr1 = ratios * advantages
            surr2 = torch.clamp(ratios, 1-self.eps_clip, 1+self.eps_clip) * advantages
            joint_loss = -logprobs.mean()*self.joint_loss_lambda
            self.loss = -torch.min(surr1, surr2) + 0.5*self.MseLoss(state_values, rewards) - 0.01*dist_entropy + joint_loss
